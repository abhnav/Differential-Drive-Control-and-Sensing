    ...
    //void overlayGrid(vector<AprilTags::TagDetection> &detections,Mat &grayImage){
      ...
      //for(int i = 0;i<r;i++){
        //for(int j = 0;j<c;j++){
          ...
          //if(grayImage[i][j] == 255 || pixelIsInsideTag(i+1,j+1,detections[robot_id].p) || pixelIsInsideTag(i+1,j+1,detections[goal_id].p) || pixelIsInsideTag(i+1,j+1,detections[origin_id].p)) world_grid[gr][gc].whites++;
...
int main(int argc, char **argv){
  ...
  imshow("what a dog sees",gray_image);
  Mat threshed;
  threshold(gray_image,threshed,100,255,0);
  cout<<"type is "<<type2str(threshed.type())<<endl;
  namedWindow("what I want",WINDOW_NORMAL);
  int r = threshed.rows, c = threshed.cols;
    for(int j = 0;j<c;j++)
      threshed.at<uint8_t>(50,j) = 0;
